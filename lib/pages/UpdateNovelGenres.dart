...
class UpdateNovelGenres extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Update Novel Genres'),
        centerTitle: true,
      ),
      body: DocumentList(),
    );
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadBookImageToStorage(
                        pickedFile.path, 'Novel_Genres_Images');
                    setState(() {
                      _imageUrl = imageUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (result != null && result.files.isNotEmpty) {
                    String pdfUrl = await uploadPdfToStorage(
                        result.files.single.path!, 'Novel_Genres_PDFs');
                    setState(() {
                      _pdfUrl = pdfUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadAuthorImageToStorage(
                        pickedFile.path, 'Novel_Author_Images');
                    setState(() {
                      _imageUrl2 = imageUrl;
                    });
                  }
