...
class UpdateNewBooks extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Update New Books'),
        centerTitle: true,
      ),
      body: DocumentList(),
    );
...
class DocumentList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: FirebaseFirestore.instance.collection('New_Books').snapshots(),
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadBookImageToStorage(
                        pickedFile.path, 'NewBooks_Images');
                    setState(() {
                      _imageUrl = imageUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (result != null && result.files.isNotEmpty) {
                    String pdfUrl = await uploadPdfToStorage(
                        result.files.single.path!, 'NewBooks_PDFs');
                    setState(() {
                      _pdfUrl = pdfUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadAuthorImageToStorage(
                        pickedFile.path, 'NewBooks_Author_Images');
                    setState(() {
                      _imageUrl2 = imageUrl;
                    });
                  }
