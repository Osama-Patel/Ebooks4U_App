...
class UpdatePop1 extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Update Popular Genres 1'),
        centerTitle: true,
      ),
      body: DocumentList(),
    );
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadBookImageToStorage(
                        pickedFile.path, 'Popular_Genres_1_Images');
                    setState(() {
                      _imageUrl = imageUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (result != null && result.files.isNotEmpty) {
                    String pdfUrl = await uploadPdfToStorage(
                        result.files.single.path!, 'Popular_Genres_1_PDFs');
                    setState(() {
                      _pdfUrl = pdfUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadAuthorImageToStorage(
                        pickedFile.path, 'Pop1_Author_Images');
                    setState(() {
                      _imageUrl2 = imageUrl;
                    });
                  }
