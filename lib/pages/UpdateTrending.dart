...
class UpdateTrendingGenres extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Update Trending Genres'),
        centerTitle: true,
      ),
      body: DocumentList(),
    );
...
class DocumentList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream:
          FirebaseFirestore.instance.collection('Trending_Genres').snapshots(),
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadBookImageToStorage(
                        pickedFile.path, 'Trending_Genres_Images');
                    setState(() {
                      _imageUrl = imageUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (result != null && result.files.isNotEmpty) {
                    String pdfUrl = await uploadPdfToStorage(
                        result.files.single.path!, 'Trending_Genres_PDFs');
                    setState(() {
                      _pdfUrl = pdfUrl;
                    });
                  }
                ...
                onPressed: () async {
                  ...
                  if (pickedFile != null) {
                    String imageUrl = await uploadAuthorImageToStorage(
                        pickedFile.path, 'Trending_Author_Images');
                    setState(() {
                      _imageUrl2 = imageUrl;
                    });
                  }
